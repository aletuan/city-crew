...
export const PRIVACY: Doc = {
  ...
  blocks: [
    ...
    { k: 'h3',
      en: 'Activity history — off by default',
      vi: 'Lịch sử hoạt động — mặc định tắt' },
    { k: 'p',
      en: 'The app can remember which places you view to improve suggestions, but **only if you switch it on** in preferences. The default is off, and the opt-in is enforced in our database, not just in the app.',
      vi: 'App có thể ghi nhớ địa điểm bạn xem để gợi ý tốt hơn, nhưng **chỉ khi bạn tự bật** trong phần tuỳ chọn. Mặc định là tắt, và việc phải-đồng-ý-trước được ép ngay ở tầng cơ sở dữ liệu, không chỉ trong app.' },
    { k: 'h2',
