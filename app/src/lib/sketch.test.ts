...
describe('deckSpan', () => {
  it('holds as many slots as the longest plan needs', () => {
    expect(deckSpan([{ stops: [1] }, { stops: [1, 2] }, { stops: [1] }])).toBe(2);
  });

  it('caps the row, however long a plan runs', () => {
    expect(deckSpan([{ stops: [1, 2, 3, 4, 5] }])).toBe(3);
    expect(deckSpan([{ stops: [1, 2, 3, 4, 5] }], 4)).toBe(4);
  });
