...
describe('openNowPlaces', () => {
  it('keeps the open doors and drops the shut ones', () => {
    const out = openNowPlaces([p('cafe', DAY, 4.5), p('bar', NIGHT, 4.8)], WED_10AM, ICT);
  ...
  it('leaves out a place whose hours it cannot read', () => {
    const out = openNowPlaces([p('mystery', null, 5), p('cafe', DAY, 4.0)], WED_10AM, ICT);
    expect(out.map((x) => x.slug)).toEqual(['cafe']);
  });
  ...
  it('discounts a five nobody has reviewed against a well-reviewed 4.9', () => {
    const out = openNowPlaces([p('thin', DAY, 5.0, 4), p('solid', DAY, 4.9, 900)], WED_10AM, ICT);
    expect(out.map((x) => x.slug)).toEqual(['solid', 'thin']);
  });

  it('breaks a dead tie on slug, so the list holds still', () => {
    const out = openNowPlaces([p('b', DAY, 4.5, 10), p('a', DAY, 4.5, 10)], WED_10AM, ICT);
  ...
  it('sorts two complete unknowns by slug alone, whichever way they arrive', () => {
    ...
    expect(openNowPlaces([bare('z'), bare('a')], WED_10AM, ICT).map((x) => x.slug))
      .toEqual(['a', 'z']);
    expect(openNowPlaces([bare('a'), bare('z')], WED_10AM, ICT).map((x) => x.slug))
      .toEqual(['a', 'z']);
  });
...
describe('taste, when there is one', () => {
  ...
  it('changes nothing at all when there is no taste', () => {
    ...
    expect(openNowPlaces(places, WED_10AM, ICT).map((p) => p.slug)).toEqual(['b', 'a']);
    expect(openNowPlaces(places, WED_10AM, ICT, 5, null).map((p) => p.slug)).toEqual(['b', 'a']);
  });
  ...
  it('reorders the pack rating leaves in a heap', () => {
    ...
    expect(openNowPlaces(places, WED_10AM, ICT).map((p) => p.slug)).toEqual(['rest', 'cafe']);
    expect(openNowPlaces(places, WED_10AM, ICT, 5, likes(['cafe'])).map((p) => p.slug))
      .toEqual(['cafe', 'rest']);
  });
  ...
  it('cannot lift a poor place over a good one', () => {
    ...
    expect(openNowPlaces(places, WED_10AM, ICT, 5, likes(['poor'])).map((p) => p.slug))
      .toEqual(['good', 'poor']);
  });
  ...
  it('marks down a place the reader walked away from', () => {
    ...
    expect(openNowPlaces(places, WED_10AM, ICT, 5, cold).map((p) => p.slug)).toEqual(['other', 'passed']);
  });
});
