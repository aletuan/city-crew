...
describe('parseSky', () => {
  ...
  // `is_day` is not on `Sky` any more — it picks the glyph and stops
  // there — so the icon is where the test can still see it working.
  it('reads is_day', () => {
...
describe('skyUrl', () => {
  ...
  // And nothing beyond them. The four the hero effects read went with
  // the effects; a field nothing parses is a field every launch waits
  // for, so the query must not quietly grow them back.
  it('asks for nothing the parser does not read', () => {
    const url = skyUrl(21.0285, 105.8542);
    for (const gone of ['precipitation', 'wind_speed_10m', 'wind_direction_10m', 'cloud_cover']) {
