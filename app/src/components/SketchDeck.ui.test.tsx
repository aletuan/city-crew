...
// as the stages tick, which is the behaviour a reader sees. What it
// cannot reach from there is the swap's own edges — a dissolve overtaken
// by a newer one, and the flip to Reduce Motion in the middle of a wait.
// Both are about landing on the right set of places, which is the only
// thing this component can get wrong.
//
...
const draw = (props: Partial<React.ComponentProps<typeof SketchDeck>> = {}) =>
  render(<SketchDeck places={[A]} seq={0} span={1} {...props} />);
...
describe('SketchDeck', () => {
  ...
  it('holds its slots when the new plan is shorter than the old one', async () => {
    ...
    rerender(<SketchDeck places={[C]} seq={1} span={2} />);
  ...
  it('lands on the newest plan when one swap overtakes another', async () => {
    const { rerender } = draw();
    rerender(<SketchDeck places={[B]} seq={1} span={1} />);
    // Inside the dissolve, so the first swap is stopped rather than run.
    await act(async () => { await vi.advanceTimersByTimeAsync(60); });
    rerender(<SketchDeck places={[C]} seq={2} span={1} />);
  ...
  it('swaps without a dissolve for a reader who turns Reduce Motion on mid-wait', () => {
    const { rerender } = draw();
    rerender(<SketchDeck places={[B]} seq={1} span={1} still />);
