...
| `09-collections-browse.yaml` | As a guest: Collections tab → first community collection → its first place → Back to the collection → Back to the list. |
...
| `CollectionsScreen` | `collection-card-<index>` (community cards, grid or rows) |
| `CollectionDetailScreen` | `collection-place-<index>`, `collection-back` (via `Screen`'s `backTestID`) |
